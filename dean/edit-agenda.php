<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $query = "
            ...
            WHERE agenda_id = ? AND department_id = ?
        ";
        $stmt = mysqli_prepare($conn, $query);
        if (!$stmt) {
            die("Database Query Failed: " . mysqli_error($conn));
        }
        mysqli_stmt_bind_param($stmt, 'ii', $id, $dean_department_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
    
        if ($row = mysqli_fetch_assoc($result)) {
            $agenda_name = htmlspecialchars($row['agenda_name']);
        ...
        $_SESSION['messages'][] = ['tags' => 'danger', 'content' => 'Invalid Agenda ID!'];
        header("Location: settings.php");
        exit();
    }
?>
        ...
        <div class="container">
            <form id="editform" method="POST" action="submit-edit-agenda.php" class="form-container">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="agenda_name" name="agenda_name" class="form-control" value="<?= htmlspecialchars($agenda_name) ?>" required>
                </div>
                <div class="mb-3">
