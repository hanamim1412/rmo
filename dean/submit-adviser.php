...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password1 !== $password2) {
        ...
        if ($stmt->execute()) {
                $_SESSION['messages'][] = ['tags' => 'success', 'content' => 'Research Adviser Added successfully.'];
                header("Location: settings.php");
                exit();
            } else {
                $_SESSION['messages'][] = ['tags' => 'danger', 'content' => 'Failed to Add Research Adviser. Please try again.'];
                header("Location: settings.php");
                exit();
            }
