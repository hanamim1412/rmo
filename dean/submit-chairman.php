...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$firstname || !$lastname || !$department_id || !$contact) {
        ...
        header("Location: add-chairman.php");
        exit();
    }
    ...
    if ($password1 !== $password2) {
        ...
        if ($stmt->execute()) {
                $_SESSION['messages'][] = ['tags' => 'success', 'content' => 'Chairman Added successfully.'];
                header("Location: settings.php");
                exit();
            } else {
                $_SESSION['messages'][] = ['tags' => 'danger', 'content' => 'Failed to Add Chairman. Please try again.'];
                header("Location: settings.php");
                exit();
            }
