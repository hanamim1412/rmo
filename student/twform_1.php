...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addProponent() {
        ...
        newProponent.classList.add('form-row', 'mt-2');

    newProponent.innerHTML = `
        <div class="form-group col-md-4">
            <input type="text" name="student_firstnames[]" class="form-control proponent" placeholder="Enter firstname" required>
        </div>
        <div class="form-group col-md-4">
    ...
    document.querySelector('.add-proponent').addEventListener('click', addProponent);
    ...
    function addTitle() {
        ...
        newTitle.classList.add('form-group', 'mt-2');
        ...
        const removeButton = newTitle.querySelector('.remove-title');
        removeButton.addEventListener('click', () => {
            newTitle.remove();
        });
    }

    document.querySelector('.add-title').addEventListener('click', addTitle);

    document.addEventListener('click', (event) => {
        if (event.target.classList.contains('remove-proponent')) {
            event.target.closest('.form-row').remove();
        } else if (event.target.classList.contains('remove-title')) {
            event.target.closest('.form-group').remove();
        }
    });

    document.getElementById('reset-button').addEventListener('click', () => {
        document.getElementById('twform1').reset();
