                            ...
                            <tbody>
                                <?php foreach ($twform5_details as $index => $twform5): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($twform5['defense_date']?? 'No assigned date yet'); ?></td>
                                        <td>
                                            <?php 
                                            $time_str = trim($twform5['time'] ?? 'no assigned time yet');  
                                            $formatted_time = DateTime::createFromFormat('H:i:s', $time_str);

                                            if ($formatted_time) {
                                                echo htmlspecialchars($formatted_time->format('g:i A')); 
                                            } else {
                                                echo "No assigned time yet"; 
                                            }
                                            ?>
                                        </td>
                                        <td><?= htmlspecialchars($twform5['place'] ?? 'no assigned place yet'); ?></td>
                                        <td><?php 
